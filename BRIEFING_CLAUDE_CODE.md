...
- **Redistribuição avisada (paciente, 24/09 — o app nunca mexe sozinho):** blocos omitidos pelo BUG 2 em dias futuros da semana corrente = espaço liberado → faixa "🔄 Você adiantou [etapa de módulo]. Sobrou espaço em [dias]. Quer preencher?" com "Ver sugestão" / "Manter como está" (`config.fillDismissed[quarta]` = assinatura do espaço liberado). `fillSuggest()` (ordem): 1) adiantar etapa de fio em andamento (pré-requisito marcado, ≥1 dia da etapa anterior, antes do dia planejado, sem violar a etapa seguinte); 2) abrir o próximo fio da fila (`fioCandidates` + pins) pela aula no 1º dia elegível, toques seguintes em dias elegíveis ou na semana seguinte (`nextOk`, pulando o sábado), selagem +3d — **a semana seguinte é avaliada com o mesmo limiar** (sinalização 2 do PR #6, 🧩 + ⚙️): `nextHours(d)` = override de `scheduleOv` ou `genForCtx(nextCtx)` da semana seguinte; um toque só cai num dia da semana seguinte se ele fica ≤ `STUDY_WARN_H`; se nenhum dia cabe, vai para o 1º dia útil (não SÁB) e a proposta diz a carga ("⚠️ QUA 21/10 (sem. seguinte) ficará em 9,1h"); dia ≥ 14 = "carga não avaliada" (o bloco rende 2 semanas depois com o rótulo "(há 2 sem.)" — `extraBlocksFor(dayIdx, extras, prevExtras, prev2Extras)`, válido também na CATCHUP/FREE; `resolveBlock` lê "(há 2 sem.)" só nos extras daquela semana, sem colidir com o "Fio N (sem. passada)" de um extra da semana anterior); 3) antecipar selagem de fio fechado com +3d atingidos; 4) "Nada cabe sem quebrar a ordem aula→questões→apostila. Espaço protegido — descanse ou adiante o Anki." Elegível = dia futuro da semana corrente que não é SÁB, nem o dia 🌿/🌬️, nem pós-noturno, nem plantão diurno, e que fica ≤ `STUDY_WARN_H` de estudo e ≤ `DAY_CAP_H` no total. Só aplica com "Aplicar": grava `config.fioPlan[quarta][slot] = { aula|quest|apost|seal: dia }` (dias 0–13; ≥7 = semana seguinte) ou `config.fioExtra[quarta] = [{ label, id, aula, quest, apost, seal }]`; `fioBlocksFor`/`fioPrevBlocksFor` renderizam pelo plano (cascata cruzada carregada; fios extras não viram fio novo — `weekFioIds` os inclui no carry). `resolveBlock` resolve "Fio N" além do ritmo pelos extras (`label`). Fio extra aberto pela sugestão aparece em **MATÉRIAS DESTA SEMANA** (aba Desatraso, `targetsAll`, slot "FIO n · sugestão") e no planejador de pomodoros (sinalização 3, 🧩) — é onde ela marca o %.
...
| Redistribuição avisada: aviso 🔄 + sugestão (adiantar etapa · abrir próximo fio · antecipar selagem · nada cabe) aplicada só com confirmação; `fioPlan`/`fioExtra`; semana seguinte conferida com o mesmo limiar (⚠️ na proposta); fio extra em MATÉRIAS DESTA SEMANA (PR #6) | ✅ |
...
35. **Redistribuição avisada (paciente, 24/09):** a paciente decide, o app nunca mexe sozinho — aviso 🔄, "Ver sugestão" mostra a proposta e só "Aplicar" grava; ordem 1) adiantar etapa em andamento 2) abrir próximo fio pela aula 3) antecipar selagem 4) "nada cabe" — nunca questões/apostila sem `aula`, nunca acima do limiar (a semana seguinte conferida com o mesmo limiar; se não cabe, a proposta avisa a carga em vez de esconder), sem sugestão em SÁB / dia 🌿 / pós-noturno; fio extra listado em MATÉRIAS DESTA SEMANA; sem migração de presenças antigas marcadas em `aula`
