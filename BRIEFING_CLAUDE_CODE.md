...
16/12+      → FREE em PIPELINE: 6 fios/semana (decisão de capacidade, 24/09) — fio novo QUA·QUI·SEX·SÁB·SEG·TER (Fio 1..6); o DOMINGO não abre fio, fecha etapas dos fios em andamento (questões do Fio 4 · apostila do Fio 3 · selagem do Fio 6 anterior); cada fio: aula → questões +1d → apostila +1d → selagem +3d; congelados por semana; simulado quinzenal mantido na sexta
```
...
|---|---|---|
| **QUA (Dia 0)** | Manhã livre: **Selar Bloco A da semana passada** (20min, +5d da apostila de sexta) → **Selar Bloco B da semana passada** (20min, +4d da apostila de sábado) → 10q pré-aula (priming) · **aula presencial A e B** (17h–22h) + D+1 relâmpago · **Selar Fio 1 (sem. passada)** (apostila foi no DOM, +3d) · ritmo 4: **Fio 4 (sem. passada) · questões** | Selagem = re-recuperação pós-critério (Rawson 2011), em blocos separados que marcam o campo do seu módulo (etiquetas A/B); pretesting (Kornell 2009); gatilho da semana |
| **QUI (Dia 1)** | **Aula online Bloco A (1.5x) → D2-A (30q) no mesmo dia** + **Fio 1 · aula** · se a semana passada foi de simulado: **banco B da semana passada** (2h, +3d da apostila de segunda) · **Selar Fio 3 (sem. passada)** (+3d) · ritmo 4: **Fio 4 (sem. passada) · apostila** | Testing effect a 1 dia da presencial (Roediger 2006) |
...
| **TER (Dia 6)** | **Banco B** (D6, +3d da apostila do B; fora de semana de simulado) + Fio 2 · apostila (ritmo 4: + Fio 4 · aula → questões QUA e apostila QUI seguintes) + faculdade 19h30–21h (até 11/10). Sem selagem (foi para a QUA) | Banco B a +3d como o banco A; nenhuma colisão com a selagem |

| **Semanas com 3+ blocos (A, B, C…)** — PR #13 | Cada **grupo** além de A e B recebe cascata própria: **C** = SÁB aula online 1.5x + D2-C (mesmo dia) → DOM apostila (+1d) → QUA' **Selar Bloco C da semana passada** (+3d, logo após as selagens de A e B) → SEX' **banco do C** (+5d — decisão 24/09: não empilhar dois bancos na QUI' após semana de simulado); **D** = um dia depois (DOM/SEG/QUI'/SÁB'). Ordem padrão: módulos principais primeiro, "Bônus" por último (`modulesForWeek`); agrupamento por semana em `config.weekGroups[semana]`; grupo de N módulos = tempo × N, "30q de cada", a marcação registra em TODOS os módulos do grupo (etiqueta = letra) | Regra de prioridade dos intervalos (decisão 40): (a) nunca duas etapas do mesmo módulo no mesmo dia · (b) questões → apostila ~1d · (c) apostila → banco/selagem ~3d · (d) aula → questões 1–2d. Bônus é complementar (🩺), mas nenhum módulo listado fica sem cascata (⚙️ + 📚) |

*Tabela reescrita em 24/09 (versão final dos espaçamentos) a partir do `FIO_PAT` vigente (§5): Fio 1 qui→sáb→dom · Fio 2 dom→seg→ter · Fio 3 sáb→dom→seg · Fio 4 ter→qua→qui. Em semana de simulado: SEX = prova + correção; SÁB = apostila do A; DOM = aula B (1.5x) + resumo + D2-B; SEG = + apostila do B + revisão adaptativa; TER = sem banco B; o banco B vai para a QUI seguinte (+3d). Cargas medidas (POST, ritmo 2 / 4): QUA 7,7 / 9,8 · QUI 6,0 / 8,2 (8,0 / 10,1 após semana de simulado, com o banco B) · SEX 6,5 / 6,5 · SÁB 4,8 / 6,5 · DOM 4,5 / 7,2 · SEG 4,6 / 6,1 · TER 4,1 / 5,8 (G2: TER 5,6 / 7,3 com a faculdade); semana de simulado: SEX 9,2 · SÁB 4,3 / 6,0 · DOM 7,8 / 10,5 · SEG 6,8 / 8,3 · TER 2,1 / 3,8.*
...
**Selagem de A e B (P6, versão de 24/09):** dois blocos fixos na QUA de manhã dos templates G2/POST, antes do priming — "Selar Bloco A da semana passada: 15 questões + smartcards + os 4 checks — registre o %" e "Selar Bloco B da semana passada: …" (20min cada): selam os blocos A e B da semana anterior (`prevBlockA`/`prevBlockB` = `modulesForWeek(week − 1)`), +5d da apostila do A e +4d da do B; `blockTasks` devolve uma tarefa `smartcard` com a etiqueta A ou B; `resolveBlock` intercepta "^Selar Bloco ([AB]) da semana passada" antes de `blockRef`. Na 1ª semana da CATCHUP a QUA recebe os dois blocos por `SEAL_AB` (fronteira). D5/D6 é a etapa 4 (banco), não selagem.
...
- **Pins (📌):** `config.catchupPins` = array de IDs fixados; toggle no botão de cada linha da fila — vale a partir da próxima semana (snapshot); na semana corrente o Cronograma oferece abrir o fixado como fio extra (decisão 47)
- **catchupForDay(dayIdx):** fallback só dos templates históricos (antigo/AMB); de 10/09 em diante todo bloco de desatraso traz "Fio N" e resolve pelo snapshot
...
```javascript
const PROJ_PACE = { amb: 1, g2: __PACE, catchup: 2, free: 6 }; // free: 6 fios/semana (decisão de capacidade); o domingo fecha etapas, não abre fio
...
```
- `lightDayFor(k)` (Schedule): dia SEM Venvanse da semana k (0 = visível; 1–3 à frente, para a sugestão) = menor `studyHours` entre os candidatos (7 dias menos a quarta com presencial `iso <= 2026-11-24`, plantões datados, pós-noturno e a sexta de simulado). **Desempate (paciente, 24/09):** cargas a ≤ 45min do mínimo (`LIGHT_TIE_H = 0.75`) formam um empate — escolher por minutos seria arbitrário — e a ordem é: (a) dia sem bloco `AULA` (faculdade/presencial) · (b) não véspera de plantão datado nem de dia-motor (segunda) · (c) menos elos de continuidade (`contLinks`: etapas do dia cuja etapa seguinte cai no dia seguinte — "Fio N · toque k" → "toque k+1", inclusive cruzando a semana, e D2-A/D2-B → apostila) · (d) o mais distante de um plantão datado · empate final: o 1º dia. Devolve `{ idx, hours, min, source, breather, tie }`; com `tie.length > 1` a interface mostra "🌿 Vários dias com carga parecida (…) — escolhi [dia]; toque para trocar." Override manual em `config.lightDayOverrides[quarta]` continua valendo (também nas semanas à frente).
...
| Princípio (24/09): a distribuição é decidida pela estratégia de aprendizagem; equalizar carga é efeito colateral. Nenhum template nasce com um dia sistematicamente vazio: G2/POST — SÁB com apostila do B + Fio 1 · questões (+ Fio 3 · aula) + PLAN, DOM com Fio 1 · apostila + Fio 2 · aula + revisão; CATCHUP — SÁB com Fio A · apostila + temas fracos + PLAN; FREE — 6 fios (DOM fecha etapas) | ✅ |
| DOM = sempre dia de estudo (FREE: recebe o fio do dia) | ✅ |
| Dia sem Venvanse calculado por semana (lightDayFor: menor carga estrita entre dias livres, empate a ≤45min resolvido por (a) sem AULA (b) não véspera de plantão/segunda (c) menos elos de continuidade (d) longe de plantão, com aviso na interface; override config.lightDayOverrides; selo 🌿) | ✅ |
...
| FREE em pipeline (aula hoje · questões ontem · apostila anteontem · selagem em bloco próprio), 6 fios/semana (sábado abre fio; domingo fecha etapas) congelados por `config.fioWeek` (PR #4) | ✅ |
...
| P3 revogado (24/09): banco B na terça (+3d) em semana normal e na QUI seguinte (+3d) após semana de simulado · P4: DOM de simulado com "(1.5x) + resumo" e "Anki dos erros" (PR #5) | ✅ |
...
| Selagem por fio em bloco próprio, +3d da apostila (G2/POST: QUA/QUI/SEX/DOM da semana seguinte; CATCHUP: Fio A na TER desta semana, Fio B na SEX seguinte ou SÁB em semana de simulado; FREE já era) + selagem de A e B na QUA de manhã da semana seguinte, blocos separados (+5d/+4d) (PR #5, PR #11) | ✅ |
...
| CATCHUP provisório: sem ambulatório Ter/Qui/Dom, sem aula da faculdade; Fio A qua→qui→sáb, Fio B dom→seg→ter, selagem do A na ter desta semana (+3d) e do B na sex seguinte (sáb em semana de simulado) (C4; sábado útil desde 24/09) | ✅ |
...
1. **FREE em pipeline (📚 tem razão; ritmo mantido):** 6 fios/semana (decisão de capacidade, 24/09): fio novo de QUA a SÁB e SEG/TER, o domingo só fecha etapas — cada dia contém a AULA do fio de hoje, as QUESTÕES do fio de ontem e a APOSTILA do fio de anteontem (1 dia + uma noite de sono entre exposição e teste; feedback do erro em ~24h). A selagem sai desse bloco e vira bloco próprio, fechando os fios cujas apostilas ocorreram há ~3 dias. Sábado continua template leve; domingo entra no pipeline.
...
8. **Princípio e correções de 24/09 (PR #10):** **a distribuição dos dias é decidida pela estratégia de aprendizagem, não pelo equilíbrio de horas** (espaçamentos inegociáveis: aula → questões ≥1d com sono; questões → apostila ~1d; apostila → banco ~3d; apostila → selagem ~3d; nunca duas etapas do mesmo módulo no mesmo dia, exceto aula online → D2 na cascata A/B; assimetria de carga é aceitável). (1) Dia leve: empate técnico (≤45min) resolvido por critérios, com aviso na interface — na bateria o resultado passou a ser TER nas semanas normais e de simulado da POST, CATCHUP e FREE (0 elos de continuidade), QUI na G2 16/09 e 23/09 (a TER tem faculdade), SEX na G2 30/09 ritmo 4, TER 13/10 na semana de plantões; SÁB só fora de empate. (2) FREE volta a 6 fios (a subida a 7 não era decisão): o domingo recebe questões do Fio 4, apostila do Fio 3 e selagem do Fio 6 anterior — todos os fios ficam com 1d/1d/3d (o desenho anterior a 24/09, que pulava o sábado, tinha 2d e 4d). **Tabela de intervalos (dias) — G2/POST normal:** Bloco A presencial →1→ aula+D2 →1→ apostila →3→ banco →1→ selagem (+4 da apostila); Bloco B presencial →2→ aula+D2 →1→ apostila →3→ banco →**0**→ selagem; Fio 1 aula →**2**→ questões →1→ apostila →3→ selagem; Fios 2, 3, 4: 1/1/3. **Semana de simulado:** A aula+D2 QUI →2→ apostila SÁB →2→ banco SEG →1→ selagem; B aula+D2 DOM →1→ apostila SEG →**1**→ banco TER →**0**→ selagem. **CATCHUP:** Fio A aula →1→ questões →**2**→ apostila →**10**→ selagem (TER da semana seguinte, "da semana passada"; antes de 24/09 era 11); Fio B 1/1/3 (5 em semana de simulado, selagem deslizada para o DOM). **FREE:** todos 1/1/3. **Sinalizados (etapas movidas por horas com espaçamento pior, ou violações herdadas):** Fio 1 questões SEX→SÁB (PR #9): aula→questões de 1d para 2d — mantém questões→apostila em 1d (a regra mais estrita); CATCHUP Fio A apostila SEX→SÁB (PR #9): questões→apostila de 1d para 2d; banco B + selagem B na mesma TER toda semana (duas etapas do mesmo módulo no mesmo dia; antes só em semana de simulado); CATCHUP selagem do Fio A a 10d da apostila (herdado); semana de simulado: banco B a 1d da apostila (herdado, P3). Decisões pendentes da paciente sobre esses pontos.
9. **Correções de espaçamento — versão final (24/09, PR #11):** (1) selagem de A e B sai da terça e vai para a manhã da QUARTA, em blocos separados e antes do priming — "Selar Bloco A da semana passada" (20min, +5d da apostila de sexta) e "Selar Bloco B da semana passada" (20min, +4d da apostila de sábado); cada uma marca o campo do seu módulo com a etiqueta A/B visível mesmo em bloco de uma tarefa só (`TaskRow` mostra a etiqueta sempre que existe); na 1ª semana da CATCHUP a quarta sela A e B da última semana do POST (fronteira). QUA passa a 7,7h (ritmo 2) / 9,8h (ritmo 4). (2) CATCHUP: "Selar Fio A" na TER desta semana (+3d da apostila de sábado), nomeando o fio; selagem do Fio B em semana de simulado desliza para o SÁB (+4d), não para o DOM (+5d). (3) Semana de simulado: banco B deixa a terça (+1d) e vai para a QUI seguinte (+3d; P3 revogado); a QUI seguinte a um simulado fica em 8,0h (ritmo 2) / 10,1h (ritmo 4), e a TER da semana de simulado em 2,1h / 3,8h. **Tabela de intervalos (dias):** G2/POST normal — A presencial →1→ aula+D2 →1→ apostila →3→ banco →2→ selagem (+5 da apostila); B presencial →2→ aula+D2 →1→ apostila →3→ banco →1→ selagem (+4); Fio 1 aula →2→ questões →1→ apostila →3→ selagem; Fios 2, 3, 4: 1/1/3. Semana de simulado — A aula+D2 QUI →2→ apostila SÁB →2→ banco SEG →2→ selagem QUA'; B aula+D2 DOM →1→ apostila SEG →**2**→ selagem QUA' →1→ banco QUI' (selagem a +2d e antes do banco — sinalizado). CATCHUP — Fio A 1/2/3; Fio B 1/1/3 (4 em semana de simulado). FREE — 1/1/3. **Decidido pela paciente (24/09):** em semana de simulado a selagem do B fica a +2d da apostila (QUA fixa) e antes do banco (QUI') — aceito como está, sem deslizar para a SEX'. A TER da semana de simulado fica em 2,1h (sem banco nem selagem) e vira o dia leve nessas semanas.

### K · BUG ESTRUTURAL — semanas com 3 blocos (paciente, 24/09) — PR #13

**Diagnóstico (confirmado por render):** a cascata usava `modulesForWeek(week)[0]`/`[1]` enquanto a lista "BLOCOS DESTA SEMANA" iterava todos os módulos. Nas semanas 8 (ANAT, PRE2, CIR2), 38 (ATB, INF1, INF2) e 43 (OFT, PSI1 I, PSI1 II) o **bônus era tratado como Bloco A**, o A verdadeiro virava B e o **B verdadeiro (CIR2 Urologia · INF2 Parasitoses · Psiquiatria II) não gerava tarefa em dia nenhum** — a tela dizia "C" e nada o estudava. Impacto: aba Módulos mostrava o card normal (sem nada que o marcasse a partir do cronograma); a fila de desatraso o recebia silenciosamente na semana seguinte (recência) e a projeção de zeramento o contava como atraso — conteúdo da semana virando desatraso. Varredura pré-correção: **6 de 45 semanas** falhavam (8, 38, 43 pelo 3º bloco; 30 pela fronteira template antigo → AMB; 32 e 34 porque o "D2 compacto" da AMB em semana de simulado não registrava a aula do B). Pós-correção: 45/45 (a 30 fica como exceção documentada com aviso ⚠️ — decisão pendente, ver relatório).

**Implementado:** decisões 41–45 (§16). **Decididas pela paciente (24/09, 2ª rodada — mesmo PR):** semana 38 com o C aceita como está (o app avisa, ela decide); banco C na SEX' (+5d) em vez de QUI'; C fica em SÁB/DOM; semana 30 recebe o D2-A na segunda 03/08 (a exceção do teste foi removida); ritmo por semana aprovado como feature, **registrado em §14 E, não neste PR**; achados da revisão geral 6–13 corrigidos (decisões 46–52).

### E · Pendências que dependem da paciente (não código)

- **Reorganização entre semanas para A/B/C e fios CATCHUP/FREE** (aval do PR #14, 24/09, sem prioridade): hoje esses blocos só se reacomodam dentro da própria semana porque o texto ("Bloco A", "Fio A") é relativo à semana em que aparece; levar um banco/apostila de A/B/C para a semana seguinte exigiria gerar sob demanda um bloco "Bloco X da semana passada" (como já existe para selagem e banco). A paciente pode mover à mão para a semana seguinte.
- **Ritmo de fios por semana** (`config.paceOverrides[isoDaQuarta]`) — aprovado como feature em 24/09, para um PR próprio: hoje o botão "Aplicar ritmo Z" muda o ritmo global (`catchupPace`); a versão por semana toca `fiosWanted`/`weekFioIds`/snapshot/`projectedZeroISO`/Planejador/CatchUp/App.
...
PLANNER_FILE=/caminho/planner_residencia_2026_offline.html PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers node audit.js        # cenários S0–S20 (425 checks em 24/09)
...
3. **Domingo = sempre dia de estudo** (G2/POST: Fio 1 · apostila + Fio 2 · aula + revisão adaptativa — a apostila do B é no sábado desde 24/09; em semana de simulado, aula B + D2-B; CATCHUP: Fio B · aula; FREE: fecha etapas — questões do Fio 4, apostila do Fio 3, selagem do Fio 6)
...
13. **CATCHUP sem ambulatório Ter/Qui/Dom e sem aula da faculdade** — provisório até a nova grade; Fio A qua→qui→sáb (sábado útil desde 24/09), Fio B dom→seg→ter, selagem do A na terça desta semana (+3d) e do B na sexta seguinte (sábado em semana de simulado)
...
18. **Reta final em PIPELINE** (📚, 14/09; capacidade confirmada em 24/09): **6 fios/semana** — fio novo QUA·QUI·SEX·SÁB·SEG·TER, o domingo fecha etapas dos fios em andamento (questões do Fio 4 · apostila do Fio 3 · selagem do Fio 6 anterior); aula hoje → questões amanhã → apostila depois de amanhã → selagem +3d, em blocos próprios; 6 módulos por semana congelados (`config.fioWeek`)
...
22. **P3 (📚):** o D6-B vai da segunda para a terça (+3d da apostila do B) — **em semana de simulado (24/09, revoga a forma original do P3) o banco B vai para a QUI seguinte (+3d), 1º dia a ≥3d sem outra etapa do módulo B**
...
25. **P6 (🩺 + 📚):** A/B ganham selagem própria — **desde 24/09 na QUA de manhã da semana seguinte, em blocos separados ("Selar Bloco A/B da semana passada", 20min cada; +5d da apostila do A, +4d da do B), antes do priming**; D5/D6 é a etapa 4 (banco), não selagem
...
37. **Regra geral (paciente, 24/09): nenhum template pode nascer com um dia sistematicamente mais vazio que os demais.** O dia leve é sempre um resultado do cálculo semanal, nunca uma escolha embutida na estrutura. Vale para G2/POST, CATCHUP e FREE. **Subordinada à decisão 38:** equalizar carga é efeito colateral, nunca objetivo.
38. **Princípio (paciente, 24/09): a distribuição dos dias é decidida pela estratégia de aprendizagem, não pelo equilíbrio de horas.** Espaçamentos inegociáveis, com prioridade sobre qualquer balanceamento: aula → questões ≥1 dia (com sono entre); questões → apostila ~1 dia; apostila → banco ~3 dias; apostila → selagem ~3 dias; nunca duas etapas do mesmo módulo no mesmo dia (exceção justificada: aula online → D2 na cascata A/B). Assimetria de carga é aceitável quando a estratégia pede. Único limite: nenhum dia nascer sistematicamente vazio a ponto de virar o dia leve por construção. Toda mudança de template deve trazer a tabela de intervalos por etapa.
39. **Dia leve — desempate por critérios (paciente, 24/09):** cargas a ≤45min entre si são "parecidas"; ordem: (a) sem aula da faculdade/compromisso fixo · (b) não véspera de plantão nem de dia-motor (segunda) · (c) menos etapas que dependem de continuidade no dia seguinte · (d) o mais distante de um plantão; a interface avisa "🌿 Vários dias com carga parecida — escolhi [dia]; toque para trocar."
41. **Semanas com 3+ blocos (paciente, 24/09 — PR #13):** a cascata suporta N grupos por semana; cada grupo recebe a cascata completa. Grupo C: SÁB aula 1.5x + D2 (mesmo dia) → DOM apostila (+1d) → QUA' selagem (+3d, junto das de A e B) → SEX' banco (+5d; decisão 24/09: dois bancos na QUI' após semana de simulado era repetição de formato sem ganho); D e seguintes: um dia depois. "Bônus" entra por último na ordem padrão (complementar, 🩺), mas recebe cascata própria se não for agrupado. Cascata genérica em todas as fases (inclusive o template antigo da semana 8).
42. **Agrupamento configurável (paciente, 24/09):** `config.weekGroups[semana] = [[ids], [ids], …]`; padrão = cada módulo no seu grupo. Grupo com N módulos: tempo × N, "30q de cada", a marcação registra a etapa em TODOS os módulos do grupo (etiqueta = letra do grupo, código no botão). Blocos adicionados/editados pela paciente (`config.customModules` ids ≥ 1000, `config.moduleEdits`) convivem com os embutidos e entram em fila, projeção, revisões e Módulos — nada é apagado (remoção só de bloco dela sem registro, com confirmação).
43. **Garantia de cobertura (paciente, 24/09):** todo módulo listado em BLOCOS DESTA SEMANA tem aula, questões e apostila marcáveis em algum dia da semana (listas reais, inclusive dias editados à mão); se faltar, aviso no topo "⚠️ [código] está na lista da semana mas não tem tarefas — agrupe-o ou defina os dias." (código repetido na semana traz o título). Teste permanente `coverage45.js` varre as 45 semanas.
44. **Capacidade em semana de 3+ blocos (paciente, 24/09):** o aviso de viabilidade existente nomeia os blocos ("Semana com 3 blocos (A, B, C — 3 cascatas completas): ritmo N não cabe…") e propõe reduzir o ritmo de fios; **nunca corta etapa de bloco principal** — conteúdo da semana tem prioridade sobre o desatraso.
45. **Etiqueta visível (24/09):** todo botão de etapa de A/B/C mostra "· letra · código" (antes só as selagens e os blocos combinados).
46. **Fios do desatraso NÃO têm etapa de banco (paciente, 24/09 — decisão explícita):** a selagem cumpre a 2ª recuperação espaçada; acrescentar banco estouraria o tempo por módulo. Não reabrir.
47. **📌 fixado no meio da semana (paciente, 24/09):** o congelamento (`config.fioWeek`) fica — previsibilidade; a regra 7 da aba Desatraso diz que o fixado assume o 1º slot **a partir da próxima semana**; quando há fixado esperando numa semana já congelada, o Cronograma mostra "📌 Você fixou … Quer abri-lo já, como fio extra?" e a sugestão de redistribuição (`fillSuggest(forceMod)`, passo 2 apenas) propõe os dias com a carga de cada um; "Deixar para a próxima semana" grava `fillDismissed[wed|pin]`.
48. **"Revisei" + % dentro do bloco de revisão do dia (paciente, 24/09):** `ReviewRows` (mesmo fluxo do card amarelo, `markReviewDone`) nos blocos REV "Revisão adaptativa/D+7/D+30" e na seção REVISÕES ESPAÇADAS; sem revisão vencida, o bloco diz "nenhuma revisão vencendo hoje" e não tem o botão.
49. **Aviso de semana em todas as fases ≥ G2 (paciente, 24/09):** na CATCHUP/FREE (ritmo travado) o aviso diz "Semana pesada: N dias acima de 8h … (ritmo travado nesta fase: K fios). Mova blocos ou deixe um fio para a semana seguinte"; sexta de simulado acima de 8h nessas fases também avisa (prova + correção + etapas de fio), sem botão de ritmo.
