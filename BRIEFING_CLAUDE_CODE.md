...
40. **Prioridade entre intervalos (paciente, 24/09):** quando dois intervalos conflitam, a ordem é (a) nunca duas etapas do mesmo módulo no mesmo dia; (b) questões → apostila em ~1 dia; (c) apostila → banco e apostila → selagem em ~3 dias; (d) aula → questões entre 1 e 2 dias. O que ceder, cede na ordem inversa. Aceitos: Fio 1 com aula → questões em 2 dias; CATCHUP Fio A com questões → apostila em 2 dias.
