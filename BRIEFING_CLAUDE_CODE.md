...
| **Semanas com 3+ blocos (A, B, C…)** — PR #13 | Cada **grupo** além de A e B recebe cascata própria: **C** = SÁB aula online 1.5x + D2-C (mesmo dia) → DOM apostila (+1d) → QUA' **Selar Bloco C da semana passada** (+3d, logo após as selagens de A e B) → QUI' **banco do C** (+4d, nunca no dia da selagem); **D** = um dia depois (DOM/SEG/QUI'/SEX'). Ordem padrão: módulos principais primeiro, "Bônus" por último (`modulesForWeek`); agrupamento por semana em `config.weekGroups[semana]`; grupo de N módulos = tempo × N, "30q de cada", a marcação registra em TODOS os módulos do grupo (etiqueta = letra) | Regra de prioridade dos intervalos (decisão 40): (a) nunca duas etapas do mesmo módulo no mesmo dia · (b) questões → apostila ~1d · (c) apostila → banco/selagem ~3d · (d) aula → questões 1–2d. Bônus é complementar (🩺), mas nenhum módulo listado fica sem cascata (⚙️ + 📚) |
...
**Implementado:** decisões 41–45 (§16). **Pendentes da paciente:** (1) semana 30 histórica — aceitar a exceção com aviso ou injetar o D2-A em 03/08; (2) ritmo de fios por semana (`config.paceOverrides`) — hoje o botão "Aplicar ritmo Z" muda o ritmo global (sugestão, não implementado); (3) colocação alternativa do C em DOM/SEG (aula+D2 a +4d da presencial, banco QUI' +3d, selagem SEX' +4d) — a implementada é SÁB/DOM (primeira recuperação mais perto da exposição, selagem exatamente +3d).
...
PLANNER_FILE=/caminho/planner_residencia_2026_offline.html PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers node audit.js        # cenários S0–S18 (383 checks em 24/09)
PLANNER_FILE=... PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers node coverage45.js   # teste permanente: 45 semanas com módulos, todo módulo listado tem aula+questões+apostila (90 checks; sem.30 = exceção documentada)
