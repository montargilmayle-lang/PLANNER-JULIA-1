...
- **Redistribuição avisada (paciente, 24/09 — o app nunca mexe sozinho; DECISÃO FINAL 24/09: a paciente decide se cabe, não o app):** blocos omitidos pelo BUG 2 em dias futuros da semana corrente = espaço liberado → faixa "🔄 Você adiantou [etapa de módulo]. Sobrou espaço em [dias]. Quer preencher?" com "Ver sugestão" / "Manter como está" (`config.fillDismissed[quarta]` = assinatura do espaço liberado). `fillSuggest()` (ordem): 1) adiantar etapa de fio em andamento (pré-requisito marcado, ≥1 dia da etapa anterior, antes do dia planejado, sem violar a etapa seguinte); 2) abrir o próximo fio da fila (`fioCandidates` + pins) pela aula no 1º dia elegível desta semana, toques seguintes por `nextOk`, selagem +3d; 3) antecipar selagem de fio fechado com +3d atingidos; 4) "Nada cabe sem quebrar a ordem aula→questões→apostila. Espaço protegido — descanse ou adiante o Anki." — **só por regra pedagógica ou dia protegido, nunca por carga**. Elegível = dia futuro da semana corrente que não é SÁB, nem o dia 🌿/🌬️, nem pós-noturno, nem plantão diurno. **O limiar de horas informa, nunca veta:** `firstDay(from, hrs, strict)` prefere o 1º dia elegível que cabe (≤ `STUDY_WARN_H` de estudo e ≤ `DAY_CAP_H` no total) e, se nenhum cabe, devolve o 1º dia elegível (⚠️); `nextOk` = 1) dia elegível desta semana que cabe · 2) dia (não SÁB) que cabe na semana onde a etapa cai (`hoursAt(d)`, d = 0–27: override de `scheduleOv` ou `genForCtx(ctxAt(k))`, até 3 semanas à frente) · 3) dia elegível desta semana com ⚠️ · 4) 1º dia útil ≥ from com ⚠️. **A proposta mostra o custo de cada dia, sempre** (`dayCost`): "aula DOM 18/10 (6,0h) · questões TER 20/10 (4,6h) · apostila ⚠️ QUA 21/10 (10,5h, sem. seguinte) · selagem ⚠️ DOM 25/10 (9,7h, sem. seguinte)" — ⚠️ acima de 8h de estudo; "(carga não avaliada)" só além de 3 semanas. **"Aplicar" está sempre disponível** (mesmo com todos os dias em ⚠️) e grava de uma vez: `config.fioPlan[quarta][slot] = { aula|quest|apost|seal: dia }` (dias absolutos; ≥7 = semanas seguintes) ou `config.fioExtra[quarta] = [{ label, id, aula, quest, apost, seal }]`; `fioBlocksFor`/`fioPrevBlocksFor` renderizam pelo plano (cascata cruzada carregada; fios extras não viram fio novo — `weekFioIds` os inclui no carry). Extras de semanas anteriores rendem por `extraBlocksFor(dayIdx, extras, prevExtras, prev2Extras, prev3Extras)` (−7 "(sem. passada)", −14 "(há 2 sem.)", −21 "(há 3 sem.)"), válido também na CATCHUP/FREE; `resolveBlock` lê "(há N sem.)" só nos extras daquela semana (sem colisão de rótulo). `resolveBlock` resolve "Fio N" além do ritmo pelos extras (`label`). Fio extra aberto pela sugestão aparece em **MATÉRIAS DESTA SEMANA** (aba Desatraso, `targetsAll`, slot "FIO n · sugestão") e no planejador de pomodoros (sinalização 3, 🧩) — é onde ela marca o %.
...
| Redistribuição avisada: aviso 🔄 + sugestão (adiantar etapa · abrir próximo fio · antecipar selagem · nada cabe só por regra pedagógica) aplicada só com confirmação; custo de cada dia à vista com ⚠️ (até 3 semanas à frente), limiar informa e nunca veta, "Aplicar" sempre disponível; `fioPlan`/`fioExtra`; fio extra em MATÉRIAS DESTA SEMANA (PR #6, PR #7) | ✅ |
...
4. **DECISÃO FINAL (24/09, PR #7) — a paciente decide se cabe, não o app:** revoga o veto por carga que o PR #6 ainda tinha (aula só em dia que coubesse; "nada cabe" quando nenhum cabia). Agora a sugestão só some por regra pedagógica ou dia protegido; cada etapa proposta mostra a carga resultante do dia (⚠️ acima de 8h, também nas semanas seguintes); "Aplicar" sempre disponível e grava tudo de uma vez; preferência por dias abaixo do limiar mantida. Sinalizações abertas (não decididas por mim): (a) dia editado à mão no Cronograma é elegível na proposta, mas o bloco gerado não aparece nele (o dia editado é dela) — decidir se a proposta deve pular dias editados; (b) na semana seguinte a proposta só pula o SÁB — o 🌿 e o pós-noturno daquela semana ainda não são evitados (o 🌿 só é escolhido quando a semana chega); (c) ordem 1→2→3 mantida mesmo quando a opção 1 cai em dia ⚠️ e a 2 caberia — a alternativa seria "proposta que cabe vence proposta que não cabe".
