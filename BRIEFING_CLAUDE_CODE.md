...
|---|---|---|
| **QUA (Dia 0)** | 10q pré-aula (priming) + **aula presencial A e B** (17h–22h) + D+1 relâmpago · **Selar Fio 1 (sem. passada)** (apostila foi no SÁB, +4d) · ritmo 4: **Fio 4 (sem. passada) · questões** | Pretesting (Kornell 2009); gatilho da semana |
...
| **SÁB (Dia 3)** | **Dia útil comum (decisão da paciente, 24/09 — substitui o "template leve")**: **Apostila do B pelos erros de ONTEM** + Fio 1 · apostila (ritmo ≥3: + Fio 3 · aula) + PLAN (intenções + recompensa + pomodoros + backup semanal) · sem academia (🏃 dia de descanso do treino) · em semana de simulado: apostila do A pelos erros de quinta. O dia **sem Venvanse** (selo 🌿) é calculado pela menor carga entre os dias livres e pode cair em qualquer dia (override por semana) | Apostila a 1 dia das questões, como o A na sexta; a semana ganha um dia de fio; reset dopaminérgico no dia de menor demanda real |
| **DOM (Dia 4)** | **Dia de fios**: Fio 2 · aula (ritmo ≥3: + Fio 3 · questões) + gym · ritmo 4: **Selar Fio 4 (sem. passada)** (+3d) · em semana de simulado: aula B (1.5x) + resumo + D2-B deslocados da sexta | Alimenta a segunda; no ritmo 2 é o dia de menor carga da semana normal (2,3h) e tende a ser o 🌿 |
...
| **TER (Dia 6)** | **Selar A e B desta semana** (30min; +4d da apostila do A, +3d da do B; em semana de simulado o banco B e a selagem caem na mesma terça, banco antes) + Fio 2 · apostila (ritmo 4: + Fio 4 · aula → questões QUA e apostila QUI seguintes) + D6-B em semana de simulado + faculdade 19h30–21h (até 11/10) | Selagem = re-recuperação pós-critério (Rawson 2011) |

*Tabela reescrita em 24/09 a partir do `FIO_PAT` vigente (§5): Fio 1 qui→sex→sáb · Fio 2 dom→seg→ter · Fio 3 sáb→dom→seg · Fio 4 ter→qua→qui. Em semana de simulado: SEX = prova + correção; SÁB = apostila do A; DOM = aula B (1.5x) + resumo + D2-B; SEG = + apostila do B; TER = D6-B (banco B). Cargas medidas (POST, ritmo 2 / 4): QUA 7,1 / 9,1 · QUI 6,0 / 8,2 · SEX 8,5 / 8,5 · SÁB 4,3 / 6,0 · DOM 2,3 / 5,0 · SEG 7,3 / 8,8 · TER 2,6 / 4,3; semana de simulado: SEX 11,2 · SÁB 3,8 / 5,5 · DOM 6,3 / 9,0.*
...
| SÁB = dia útil comum na G2/POST (apostila do B + Fio 1 · apostila + Fio 3 · aula + PLAN/backup; decisão 24/09) · template leve só na CATCHUP/FREE | ✅ |
...
2. **Sábado = template leve por construção** em CATCHUP/FREE — **REVOGADO para G2/POST em 24/09 (decisão 36)**: lá o sábado é dia útil comum; o dia SEM Venvanse é calculado (decisão 15)
...
36. **Sábado útil na G2/POST (paciente, 24/09):** o sábado deixa de nascer vazio — recebe a apostila do B pelos erros de ontem (em semana de simulado, a do A), o Fio 1 · apostila e, no ritmo ≥3, o Fio 3 · aula (`FIO_PAT`: 1 qui→sex→sáb · 2 dom→seg→ter · 3 sáb→dom→seg · 4 ter→qua→qui), mantendo PLAN + backup e sem academia; o domingo fica só com os fios (+ aula B/D2-B em semana de simulado). O dia leve é calculado de verdade (menor carga estrita, sem desempate do sábado). Selagem do Fio 1 continua na QUA (+4d). CATCHUP/FREE seguem com o sábado leve (não decidido).
